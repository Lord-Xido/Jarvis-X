...
def instruction(opcode, dst=0, src1=0, src2=0, imm=0):
    return SimpleNamespace(
        opcode=opcode, dst=dst, src1=src1, src2=src2, imm=imm
    )
...
def test_checkpoint_restore_is_transactional():
    substrate = ElectronicSubstrate()
    checkpoint = substrate.checkpoint()
    substrate.tick(instruction(0x01, imm=1), {"A": 0}, {"A": 1})
    substrate.restore(checkpoint)
    assert substrate.cycle == 0
    assert substrate.trace == []
    assert substrate.cumulative_energy_j == 0.0

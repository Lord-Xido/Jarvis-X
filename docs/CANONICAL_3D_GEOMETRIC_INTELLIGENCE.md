...
## 17. Intrinsic Riemannian specialization

The coordinate-free specialization is defined in
...
be the observation and latent manifolds. The encoder is a smooth map
\(E_\theta:\mathcal X\to\mathcal Z\), or an immersion / embedding of an
intrinsic data manifold \(\mathcal M\subset\mathcal X\) when the dimension
condition permits.

The inward operation is an intrinsic flow generated by a latent potential
\(U_t:\mathcal Z\to\mathbb R\):
...
\(\Omega_t\) for residual / temporal memory.

The reconstruction and intrinsic residual are
...
The decoder differential and its metric adjoint define a natural latent
correction:
...
The candidate update is reinjected intrinsically by
...
or by a declared retraction \(R_Z\) when exact exponential-map evaluation is
unnecessary or too expensive.

The continuous controlled latent law is
...
An optional adaptive-metric backend may additionally evolve

\[
\partial_t g_t
=
-2\operatorname{Ric}(g_t)+\mathcal S_t,
\]

subject to positive-definiteness, conditioning, resource, reconstruction and
evidence gates.

This specialization makes "inward" an intrinsic geometric statement:
descent toward a declared low-energy / stable region under \(g_t\), rather
than mere shrinkage of Cartesian rendering coordinates. The existing
dependency-free implementation remains the flat \(G=I\) reference case.
